...
//TYPE_R
  ...
  INSTPAT("0000000 ????? ????? 101 ????? 01100 11", srl    , R, R(dest) = (uint32_t)(src1 >> src2));
  INSTPAT("0100000 ????? ????? 101 ????? 01100 11", sra    , R, R(dest) = (int32_t)(src1) >> src2);
//TYPE_32R
  INSTPAT("0000001 ????? ????? 000 ????? 01100 11", mul    , R, R(dest) = src1 * src2);
  INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh   , R, R(dest) = SEXT(BITS((int64_t)src1 * (int64_t)src2, 63, 32), 32));
  INSTPAT("0000001 ????? ????? 010 ????? 01100 11", mulhsu , R, R(dest) = (int32_t)BITS((int64_t)src1 * (uint64_t) src2, 63,32));
  INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu  , R, R(dest) = (int32_t)BITS((uint64_t)src1 * (uint64_t) src2, 63,32)); 
...
//TYPE_I
  ...
  INSTPAT("0000000 ????? ????? 101 ????? 00100 11", srli   , I, R(dest) = (uint32_t)(src1) >> imm);
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I, R(dest) = (int32_t)(src1) >> (imm & 0x1f));
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, R(dest) = s->pc + 4; s->dnpc = ((src1 + imm) & (~(uint32_t)(1))));
...
//TYPE_CSR
  //INSTPAT("??????? ????? ????? 010 ????? 11100 11", csrrs , I, word_t t = csr(imm); csr(imm) = t | src1; R(dest) = t);
  //INSTPAT("??????? ????? ????? 010 ????? 11100 11", csrrs  , I, word_t t = csr(imm); csr(imm) = t | src1; R(dest) = t);
//
